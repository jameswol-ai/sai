...
import random
import logging
import yfinance as yf   # NEW: Yahoo Finance API
...
def generate_trade():
    ...
    # Use active ML model if available
...
def strategy_config_tab():
    ...
    if st.button("Update Strategy"):
        ...
        st.success(f"Updated thresholds: BUY<{buy_threshold}, SELL>{sell_threshold}, Ticker={ticker}")
...
# --- Main App ---
def main():
    st.title("SAI Trading Bot (Live AI-Driven)")
    ...
    if tab == "Dashboard": dashboard time
import pandas as pd
import matplotlib.pyplot as plt
import pickle
import random
